...
> **Native bundle development:** the `feature/native-web-bundle` branch packages Cheating Case Review as a routable Razor page in `DemosToDiscord.zip`. Reliable automatic queue/case navigation, the custom sidebar icon and host-themed responsive styling are supplied by the bundle. Evidence capture, proactive detection, Discord delivery and moderation decisions remain unchanged. See [Native web bundle development](docs/Native-Web-Bundle.md).
